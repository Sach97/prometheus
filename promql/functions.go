...
// === ichange(node model.ValMatric) Vector ===
func funcIchange(vals []Value, args Expressions, enh *EvalNodeHelper) Vector {
	return variationValue(vals, enh.out, false)
}

func variationValue(vals []Value, out Vector, isRate bool) Vector {
	for _, samples := range vals[0].(Matrix) {
		...
		resultValue = (lastSample.V - previousSample.V) / previousSample.V
